...
function display(d) {
    ...
    .on("mouseover", function(d) { 
        ...
        //map mouse over function here
        console.log("mouseover rect : " + d.name);
    })
    .append("title")
    .text(function(d) {
        return d.name + " " + formatNumber(d.value);
    ...
    .on("mouseover", function(d) { 
        ...
        //duplicate map mouse over function here
        console.log("mouseover text : " + d.name);
    ...
    function transition(d) {
        ...
        var tempsubname = name(d).substring(name(d).lastIndexOf(".")+1);
        var templevel = name(d).split(".").length;
        //plug in map zoom here
        // console.log(_regionmap[d.name]==null);
        if(_regionmap[tempsubname]!=null){
            _regionname=tempsubname;
            mapfocus();
        }                 
             
        //filter here
        if(d.parent!=null){
            if(d.parent.name=="Terrorism"){
                //zoom back to full map
                _regionname=null;
                mapfocus();  
            }
        }
        ...
        if(templevel===2){
            //filter here
            _attackType = tempsubname;
            writeFilter();
            showworldmap();
            d3.select("#bar2").select("svg").remove();
            drawBar(tempsubname);
